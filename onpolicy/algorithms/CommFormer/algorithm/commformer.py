...
from onpolicy.algorithms.utils.commformer_act import discrete_autoregreesive_act
from onpolicy.algorithms.utils.commformer_act import discrete_parallel_act
from onpolicy.algorithms.utils.commformer_act import continuous_autoregreesive_act
from onpolicy.algorithms.utils.commformer_act import continuous_parallel_act
...
class CommFormer(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
        ...
        torch.nn.init.uniform_(self.edges_embed.weight, -1.0, 1.0)
    ...
    def forward(self, state, obs, action, available_actions=None, steps=0, total_step=0):
        ...
        relations_embed = self.edges_embed(relations.round().long())
        relations_embed = relations_embed.expand(batch_size, -1, -1, -1).clone()  # 1 x n x n x emd
        ...
        if self.action_type == 'Discrete':
            action = action.long()
            action_log, entropy = discrete_parallel_act(self.decoder, obs_rep, obs, action, relations_embed, relations,
                                                        batch_size,
                                                        self.n_agent, self.action_dim, self.tpdv, available_actions,
                                                        dec_agent=dec_agent)
        else:
            action_log, entropy = continuous_parallel_act(self.decoder, obs_rep, obs, action, relations_embed,
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False):
        ...
        relations_embed = relations_embed.expand(batch_size, -1, -1, -1).clone()
        ...
        if self.action_type == "Discrete":
            output_action, output_action_log = discrete_autoregreesive_act(self.decoder, obs_rep, obs, relations_embed,
                                                                           relations, batch_size,
                                                                           self.n_agent, self.action_dim, self.tpdv,
                                                                           available_actions, deterministic,
                                                                           dec_agent=dec_agent)
        else:
            output_action, output_action_log = continuous_autoregreesive_act(self.decoder, obs_rep, obs,
                                                                             relations_embed, relations, batch_size,
                                                                             self.n_agent, self.action_dim, self.tpdv,
