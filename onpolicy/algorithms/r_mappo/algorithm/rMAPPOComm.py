import torch
from onpolicy.algorithms.r_mappo.algorithm.r_actor_critic_comm import R_Actor_Comm, R_Critic_Comm
...
class R_MAPPOComm:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        self.onoff = True if args.comm_type == 1 and args.algorithm_name == 'rmappo_comm' else False

        self.obs_space = obs_space
        self.share_obs_space = cent_obs_space
        self.act_space = act_space

        self.actor = R_Actor_Comm(args, self.obs_space, self.act_space, self.device)
        self.critic = R_Critic_Comm(args, self.share_obs_space, self.device)
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None, comm_graphs=None,
        ...
        if self.onoff:
            ...
            values, rnn_states_critic, vq_loss_q = self.critic(cent_obs, rnn_states_critic, masks, comm_graphs)
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks, comm_graphs=None):
        ...
        :return values: (torch.Tensor) value function predictions.
        """
        values, _, _ = self.critic(cent_obs, rnn_states_critic, masks, comm_graphs)
        return values
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, action, masks,
        ...
        if self.onoff:
            ...
            values, _, vq_loss_q = self.critic(cent_obs, rnn_states_critic, masks, comm_graphs)
