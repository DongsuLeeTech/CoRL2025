...
def split_observations(obs):
    image = {agent: data['image'] for agent, data in obs.items()}
    state = {agent: data['state'] for agent, data in obs.items()}
    return image, state
...
class MetaDriveEnv(object):
    ...
    def __init__(self, args):
        ...
        for key in self.vehicle_keys:
            ...
            if self.lidar_pt_cloud:
                ...
                # self.observation_dim = self.env.observation_space[k]['state'].shape[0] - self.lidar_num_lasers
            if self.global_pos:
    ...
    def reset(self):
        obs = self.env.reset()
        # image, state = split_observations(obs)
        self.vehicle_keys = list(self.env.vehicles.keys())
        self.active_agents = list(self.env.agent_manager.active_agents.keys())
        self.dead_agents = []
        self.dead_ids = []
    ...
    def step(self, action):
        ...
        # image, obs = split_observations(obs)
        ...
        obs = self._obs_wrapper(obs)
        return obs, reward, done, info#, image
