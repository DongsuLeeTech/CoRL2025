...
def record_video(label, renders=None, n_cols=None, skip_frames=1, filename='test.gif'):
    ...
    return save_video(label, renders, n_cols=n_cols, filename=filename)

def save_video(label, tensor, fps=15, n_cols=1, filename='test.gif'):
    def _to_uint8(t):
    ...
    fig, ax = plt.subplots()
    frames = []  # Artist 객체를 저장할 리스트

    for i in range(num_frames):
        img = ax.imshow(tensor[i].transpose(1, 2, 0), animated=True)
        frames.append([img])  # 리스트로 감싸서 `ArtistAnimation`에 맞게 저장

    ani = animation.ArtistAnimation(fig, frames, interval=1000//fps, repeat_delay=1000, blit=True)
    ani.save(filename, fps=15)
    return wandb.Video(tensor, fps=fps, format='gif')
...
def save_gif_with_graph(graph, slicin_idx = 1, filename="graphs.gif"):
    ...
    for t in range(num_timesteps):
        ...
        sns.heatmap(tmp, annot=True, cmap="viridis", cbar=True, ax=ax,
                    square=True, cbar_kws={'shrink': .8}, vmin=0, vmax=1)
...
def record_gif(label, renders=None, n_cols=None, skip_frames=5, filename="output.gif"):
    max_length = max([len(render) for render in renders])
    ...
    renders = np.array(renders)
    gif_frames = prepare_video_gif(renders, n_cols)

    # GIF 저장
    imageio.mimsave(filename, gif_frames, duration=0.1)
    print(f"GIF saved as {filename}")

    return filename


def prepare_video_gif(v, n_cols=None):
    orig_ndim = v.ndim
    if orig_ndim == 4:
        v = v[None,]
    ...
    if v.shape[0] % n_cols != 0:
        len_addition = n_cols - v.shape[0] % n_cols
        v = np.concatenate((v, np.zeros((len_addition, t, c, h, w))), axis=0)

    n_rows = v.shape[0] // n_cols
    v = np.reshape(v, newshape=(n_rows, n_cols, t, c, h, w))
    v = np.transpose(v, axes=(2, 0, 4, 1, 5, 3))  # (t, n_rows, h, n_cols, w, c)
    v = np.reshape(v, newshape=(t, n_rows * h, n_cols * w, c))

    # 이미지 리스트 생성
    gif_frames = [Image.fromarray((frame * 255).astype(np.uint8)) for frame in v]
    return gif_frames
