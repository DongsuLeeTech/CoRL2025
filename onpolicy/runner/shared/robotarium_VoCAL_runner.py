...
class RobotariumRunner(Runner):
    """Runner class to perform training and evaluation for Robotarium environment."""
    ...
    def warmup(self):
        ...
        self.buffer.available_actions[0] = np.ones((self.n_rollout_threads, self.num_agents, self.envs.action_space[0].n))

    @torch.no_grad()
    def collect(self, step):
        self.trainer.prep_rollout()
        value, action, action_log_prob, rnn_state, rnn_state_critic, _, _, _, _ \
    ...
    def eval(self, total_num_steps):
        ...
        for eval_episode in range(self.num_eval_episodes):
            ...
            eval_comm_graphs = np.ones((self.n_eval_rollout_threads, self.num_agents, self.num_agents),
                                        dtype=np.float32)
            eval_episode_step = []
            eval_episode_reward = []
            ...
            for eval_step in range(self.episode_length):
                ...
                eval_action, eval_rnn_states, _, _, _, graph = self.trainer.policy.act(np.concatenate(eval_obs),
            ...
            eval_episode_steps.append(eval_episode_step[0])

        # Convert to numpy array
        # video = record_video('Video', renders=np.array(all_frames))
    ...
    def save(self, episode):
        torch.save(self.trainer.policy.actor.state_dict(), str(self.save_dir) + "/actor.th")