...
import uuid
import psutil
from onpolicy.utils.tarmac_buffer import SharedReplayBuffer
...
class MetaDriveRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                if len(actions_env.shape) == 4:
                    actions_env = actions_env.squeeze(1)
    ...
    def insert(self, data):
        ...
        self.buffer.insert(share_obs, obs, actions, action_log_probs, values, rewards, masks, comm_graphs=comm_graphs)
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            eval_obs, eval_rewards, eval_dones, eval_infos = self.eval_envs.step(eval_actions_env.squeeze(1))
    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            for step in range(self.episode_length):
                ...
                self.trainer.prep_rollout()
                action = self.trainer.policy.act(obs,
    ...
    def save(self, episode):
        policy_agent = self.trainer.policy.agent
        torch.save(policy_agent, str(self.save_dir) + "/agent.pt")
    ...
    def restore(self):
        ...
        if "mat" in self.algorithm_name:
            ...
            self.policy.agent = torch.load(str(self.model_dir) + '/agent.pt')
            if self.trainer._use_valuenorm:
