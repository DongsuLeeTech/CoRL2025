...
class MetaDriveRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.eval_interval == 0 and self.use_eval:
                reward = self.eval(total_num_steps)
                if reward > self.best_reward:
                    self.best_reward = reward
                    self.save_best(episode)
    ...
    def eval(self, total_num_steps):
        ...
        return np.mean(np.sum(np.array(eval_episode_rewards), axis=0))
    ...
    def save_best(self, episode):
        policy_actor = self.trainer.policy.actor
        torch.save(policy_actor, str(self.save_dir) + "/actor_best.pt")
        policy_critic = self.trainer.policy.critic
        torch.save(policy_critic, str(self.save_dir) + "/critic_best.pt")
        if self.trainer._use_valuenorm:
            policy_vnorm = self.trainer.value_normalizer
            torch.save(policy_vnorm, str(self.save_dir) + "/vnorm_best.pt")