...
class MetaDriveRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                train_infos["normalized_average_batch_episode_rewards"] = np.mean(
                    np.sum(self.buffer.rewards, axis=-2).squeeze() / num_all_agents) * self.episode_length
                print("average batch episode rewards is {}".format(train_infos["average_batch_episode_rewards"]))
                print("normalized average batch episode rewards is {}".format(
                    train_infos["normalized_average_batch_episode_rewards"]))
    ...
    def collect(self, step):
        ...
        if not "comm" in self.algorithm_name and not "mat" in self.algorithm_name:
            ...
            value, action, action_log_prob, rnn_states, rnn_states_critic, actor_comm, att, att_mask, graphs \
                = self.trainer.policy.get_actions(np.concatenate(self.buffer.share_obs[step]),
                                                  np.concatenate(self.buffer.obs[step]),
                                                  np.concatenate(self.buffer.rnn_states[step]),
                                                  np.concatenate(self.buffer.rnn_states_critic[step]),
    ...
    def insert(self, data):
        ...
        # comm_graph: [n_rollout_threads, num_agents, num_agents]
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            if not "comm" in self.algorithm_name and not "mat" in self.algorithm_name:
                eval_action, eval_rnn_states = self.trainer.policy.act(np.concatenate(eval_obs),
                                                                       ...
                                                                       deterministic=True)
            elif "comm" in self.algorithm_name:
                eval_action, eval_rnn_states, comm_info, att, att_mask, graph = self.trainer.policy.act(
                ...
                eval_action, eval_rnn_states = self.trainer.policy.act(np.concatenate(eval_obs),
                                                                       ...
                                                                       comm_graphs=eval_comm_graphs,
                                                                       deterministic=True)
            if self.algorithm_name == 'rmappo_comm':
        ...
        if self.algorithm_name == 'rmappo_comm':
            ...
            att_viz = save_gif_with_attention(att_list, slicin_idx=100,
                                              filename=f'./tmp_data/attention_{self.my_uuid}.gif')
            mask_viz = save_gif_with_attention(att_masks, slicin_idx=100,
                                               filename=f'./tmp_data/mask_{self.my_uuid}.gif')
        ...
        # np.save(f'./results/actor_comm_{total_num_steps}', comm_list)
        ...
        eval_env_infos = {}
        eval_env_infos['rew/eval_average_batch_episode_rewards'] = [
            np.mean(np.sum(np.array(eval_episode_rewards), axis=0))]
        print("eval average batch episode rewards of agent: " + str(
            eval_env_infos['rew/eval_average_batch_episode_rewards'][0]))
        eval_env_infos['rew/eval_normalized_average_batch_episode_rewards'] = [
            np.mean(np.sum(np.sum(np.array(eval_episode_rewards), axis=0).squeeze(), axis=-1) / num_all_agents)]
        print("eval normalized average batch episode rewards of agent: " + str(
            eval_env_infos['rew/eval_normalized_average_batch_episode_rewards'][0]))
        ...
        eval_env_infos['eval/rendering'] = video  # wandb.Video(np.array(all_frames), fps=10, format="gif")
        if self.algorithm_name == 'rmappo_comm':
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            for step in range(self.episode_length):
                ...
                if not "comm" in self.algorithm_name and not "mat" in self.algorithm_name:
                    ...
                    # mat, the first input could be cleaned, leave it for now
                    action, rnn_states = self.trainer.policy.act(np.concatenate(obs),
                                                                 np.concatenate(obs),
                                                                 np.concatenate(rnn_states),
                                                                 np.concatenate(masks),
            ...
            print("rendering average (real) episode reward is: " + str(
                np.mean(np.sum(np.array(episode_rewards), axis=0))))
            print("rendering average success is: " + str(np.mean(np.array(success_rates))))

        # with open(f'reward_trajs.pkl', 'wb') as handle:
        #     pickle.dump(all_rewards, handle, protocol=pickle.HIGHEST_PROTOCOL)
        # with open(f'reward_trajs.pkl', 'rb') as handle:
